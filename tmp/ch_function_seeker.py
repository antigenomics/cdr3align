...
def cdrantigen_distances(inppath = 'seqdata/HomoSapiens/cdrs/cdr3.annotation.txt',
    ...
    with open(outpath, 'w') as out:
        out.write('pdb\tdistance\tCDR_chain\tAA_cdr\tAA_ant\tposcdr\tposant\tcdr_seq\tant_seq\tascordesc\tmhc_type\n')
            ...
            for allele in pdb[1].groupby('chain_allele'):
                ...
                if len(work_atoms)%2 == 0:
                    ...
                    for i in range(len(work_atoms)):
                        atom, dist = get_min_distance(work_atoms[i], antigen_atoms, ca)
...
def cdrantigen_min_distances(inppath='seqdata/HomoSapiens/distances/cdr_and_antigen.txt', outpath='seqdata/HomoSapiens/distances/min_cdr_and_antigen.txt'):
    ...
    with open(outpath, 'w') as out:
        out.writelines('pdb\tdistance\tCDR_chain\tAA_cdr\tAA_ant\tposcdr\tposant\tcdr_seq\tant_seq\tascordesc\tmhc_type\tcdr_len\n')
        for pdb in pdbs:
...
cdrantigen_distances(myargs.input_dir, os.path.join(myargs.output_dir, 'cdr_and_antigen.txt'), myargs.pdb, myargs.ca)
