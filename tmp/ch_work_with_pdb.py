...
parser.add_argument("-cdr", help="CDR type. Default is 'CDR3'", required=False, dest='cdr')
parser.add_argument("-o", help="Path to output directory. Default is '../seqdata/HomoSapiens/distances_from_pdb_'+"
                               "'chain name' + 'cdr type' + '.txt'",
...
if myargs.chain is None:
    myargs.chain = 'B'
if myargs.cdr is None:
    myargs.cdr = 'CDR3'
if myargs.output_dir is None:
    myargs.output_dir = '../seqdata/HomoSapiens/distances_from_pdb_'+str(myargs.chain)+str(myargs.cdr)+'.txt'
