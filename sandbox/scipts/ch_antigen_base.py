...
def test_and_teach(seq_dict, limit, times, filename):
...
how size of test sample is lower than quantity of all sequences"""
    ...
    with open(filename + '_test_antigen', 'w') as out:
        for i in seq_dict_test:
            out.write('\n'.join(list(map(lambda x: str(x) + '\t' + str(i), seq_dict_test[i]))) + '\n')
    with open(filename + '_teach_antigen', 'w') as out:
        for i in seq_dict_teach:
            out.write('\n'.join(list(map(lambda x: str(x) + '\t' + str(i), seq_dict_teach[i]))) + '\n')
...
def get_info_from_matrices(outfolder, alignms):
    ...
    with open(os.path.join(outfolder,'sum_of_info_from_alignments'), 'w') as out:
        writer = csv.writer(out, delimiter='\t')
...
def get_alignments(infname):
    alignments = {}
    with open(os.path.join(infname), 'r') as inp:
        for line in inp:
